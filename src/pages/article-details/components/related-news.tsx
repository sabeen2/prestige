...
import { Clock, Eye, Bookmark, Share2, Calendar, User } from "lucide-react";
import { initGetRelatedArticlesByCategorySlug } from "@/services/articles/request";
  ...
  useEffect(() => {
    const fetchRelatedArticles = async () => {
      if (!articleId || !categorySlug) {
        setError("Article ID or category slug is missing");
        setLoading(false);
        return;
      }
      ...
      try {
        setLoading(true);
        const relatedArticlesResponse: ApiResponse =
          await initGetRelatedArticlesByCategorySlug({
            articleId,
            categorySlug,
          });
